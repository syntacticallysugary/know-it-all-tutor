"""Shared cleanup prompt template — single source of truth for all generate_prompts.py files.
Parameters: idx, slug, display_name, term_count, base, data_dir
"""

TEMPLATE = """\
# Definition Cleanup Agent — {display_name} ({term_count} terms)
...
This file covers **{display_name}** — **{term_count} terms**.
...
| Role | Path |
|------|------|
| Source (read this) | `{base}/{data_dir}/domain_{idx:02d}_{slug}.json` |
| Output (write this) | `{base}/{data_dir}/domain_{idx:02d}_{slug}_fix.json` |
...
1. **Read** `{base}/{data_dir}/domain_{idx:02d}_{slug}.json` using the Read tool.

2. **Work through all {term_count} terms** in order. For each term:
...
5. **Write** the complete modified JSON to
   `{base}/{data_dir}/domain_{idx:02d}_{slug}_fix.json` using the Write tool.
...
7. **Report**: state how many terms you inspected, how many definitions you
   changed, and how many you left unchanged. The total must equal {term_count}.
