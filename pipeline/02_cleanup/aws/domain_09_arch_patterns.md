# Definition Cleanup Agent — AWS — Architectural Patterns (11 terms)
...
This file covers **AWS — Architectural Patterns** — **11 terms**.
...
| Role | Path |
|------|------|
| Source (read this) | `/home/jimbob/Dev/AWS_Dev/data/aws_domains/domain_09_arch_patterns.json` |
| Output (write this) | `/home/jimbob/Dev/AWS_Dev/data/aws_domains/domain_09_arch_patterns_fix.json` |
...
{
  "domain_index": 9,
  "domain_name": "AWS — Architectural Patterns",
  "term_count": 11,
  "terms": [
    {
      "node_type": "term",
...
| Source | Derive |
|--------|--------|
| Full name | Lowercase version: `"EC2 Auto Scaling"` → `"ec2 auto scaling"` |
| Abbreviation in parentheses | `"Service Control Policy (SCP)"` → also match `SCP`, `SCPs` |
| Plural / singular | `"Spot Instance"` → also match `"Spot Instances"` |
| AWS/Amazon prefix | `"Amazon ECS"` → also match `"ECS"`, `"ecs"` |
| All of the above | case-insensitive: `"Transit Gateway"` matches `"transit gateway"`, `"TRANSIT GATEWAY"` |

**Write out the lowercase form explicitly.** Definitions naturally use lowercase
mid-sentence. `"Elastic Beanstalk"` → definitions will say `"elastic beanstalk
manages the environment"` — this is still a match and must be rewritten.
...
**Variants:** `ecs`, `elastic container service` (all case-insensitive)

**Source (3 sentences):**
> "ECS is a fully managed container orchestration service. The core abstractions
> are task definitions and clusters. ECS integrates natively with IAM task roles."

**Sentence-by-sentence scan:**
- Sentence 1: contains `ECS` → **REWRITE**
  → `"A fully managed container orchestration service."`
- Sentence 2: no term → **COPY UNCHANGED**
- Sentence 3: contains `ECS` → **REWRITE**
  → `"Integrates natively with IAM task roles."`

**Output:**
> "A fully managed container orchestration service. The core abstractions are
> task definitions and clusters. Integrates natively with IAM task roles."
...
|---------|--------|-------|
| `"A X is ..."` | `"A Transit Gateway is a hub..."` | `"A hub that..."` or `"Centralized hub..."` |
| `"X is a ..."` | `"Lambda is a serverless service..."` | `"A serverless service..."` |
| `"X are ..."` | `"Spot Instances are spare EC2..."` | `"Spare EC2..."` |
| `"An X is ..."` | `"An IAM role is an identity..."` | `"An identity..."` |
| `"The X ..."` | `"The Transit Gateway acts as a hub..."` | `"Acts as a hub..."` |
| `"X (ABBREV) ..."` | `"A Service Control Policy (SCP) defines..."` | `"Defines the maximum permissions..."` |
| `"X refers to ..."` | `"Elastic Beanstalk refers to..."` | `"The platform that..."` |
...
| `Service Control Policy (SCP)` | `SCP`, `SCPs` |
| `EC2 Auto Scaling` | `Auto Scaling` (when referring to this service) |
| `Elastic Beanstalk` | `Beanstalk` (informal short form used in definitions) |
...
- Match plural/singular: `"instance"` ↔ `"instances"`.
- If the abbreviation appears alone in a later sentence (e.g., `SCP`, `ECS`),
...
- Term `AWS Lambda`: a definition may legitimately mention `Lambda Layers` as a related feature — do not remove it.
- Term `Transit Gateway`: occurrences of `VPN gateway` refer to a different service — do not remove.
...
1. **Read** `/home/jimbob/Dev/AWS_Dev/data/aws_domains/domain_09_arch_patterns.json` using the Read tool.

2. **Work through all 11 terms** in order. For each term:
...
5. **Write** the complete modified JSON to
   `/home/jimbob/Dev/AWS_Dev/data/aws_domains/domain_09_arch_patterns_fix.json` using the Write tool.
...
7. **Report**: state how many terms you inspected, how many definitions you
   changed, and how many you left unchanged. The total must equal 11.
