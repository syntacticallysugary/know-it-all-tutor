# Definition Cleanup QA Agent — Java Domain

## Role

Audit the cleaned Java definition file against the original. For every term,
...
| Role | Path |
|------|------|
| Original | `/home/jimbob/Dev/AWS_Dev/data/java_upload.json` |
| Fixed | `/home/jimbob/Dev/AWS_Dev/data/java_upload-fix.json` |
...
1. `/home/jimbob/Dev/AWS_Dev/data/java_upload-issues.json`
...
Java terms have several naming forms — all count as matches:
- The full term string (case-insensitive), e.g., `java.util.Optional`
- The simple class name without package prefix, e.g., `Optional`
- Annotation terms without the `@`, e.g., term `@Override` → also match `Override`
