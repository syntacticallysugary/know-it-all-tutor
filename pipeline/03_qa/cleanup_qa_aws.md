# Definition Cleanup QA Agent — AWS Domain

## Role

Audit the cleaned AWS definition file against the original. For every term,
...
| Role | Path |
|------|------|
| Original | `/home/jimbob/Dev/AWS_Dev/data/aws_upload.json` |
| Fixed | `/home/jimbob/Dev/AWS_Dev/data/aws_upload-fix.json` |
...
1. `/home/jimbob/Dev/AWS_Dev/data/aws_upload-issues.json`
...
AWS terms have several naming forms — all count as matches:
- The full term string (case-insensitive), e.g., `Amazon S3`, `AWS Lambda`
- The short name without the `AWS`/`Amazon` prefix, e.g., `S3`, `Lambda`
- Any acronym found in a parenthetical in the term string,
  e.g., `Service Control Policy (SCP)` → also match `SCP`, `SCPs`
